...
const NEZHA_KEY = process.env.NEZHA_KEY || 'Ry4zCm8V0zREBQx7Aa';
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || '';     // 
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiMjIxMDJlN2YyMjA3MTE1ZDY1ODc3MTMxNGNiMjlhM2EiLCJ0IjoiMzg5MDhiZmItY2VjYS00MThmLTgzOWEtYjY4MWQzYzU0ZGVmIiwicyI6IllURm1OelEyT1RZdE5UVXhOUzAwTXpVMExUZzRNek10TkRRME16YzVPV1prTjJWbSJ9';
const CFIP = process.env.CFIP || 'gt.shc';
const NAME = process.env.NAME || 'xm78';
...
// run-nezha
  ...
  const command = `nohup ./swith -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --report-delay 2 >/dev/null 2>&1 &`;
  try {
    exec(command);
